// classes are like template for creating javascript objects. they can inherit existing prototypes of objects. they usually get initialized with a constructor.
...
class AppError extends Error {
  constructor(message, statusCode = 500) {
    ...
    this.status = `${statusCode}.startwith("4") ? "fail" : "error"`; // we want to determine the error type, if it starts witn 4, then assign a status of fsil, otherwise we assign a status of error
    this.success = false; // we default to false in this case because we are handling errors
    this.isOperational = true // distinguish between operational errors, such as server shutdwon or connection, validation errors, authentication errors while programmer errors should not be sent to the client - bugs, syntax  or type errors.
  }
}
