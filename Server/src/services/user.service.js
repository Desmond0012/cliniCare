...
const userService = {
  register: async (userData, next) => {
    //UserData is parameter. check if email already exists and here we all looking at the user model
    const emailExists = await User.findOne({ email: userData.email });
    if (emailExists) {
      return next(errorResponse("Email already exists", 400));
    }
    //if fresh new userData(email i.e that is if the email is new), then we proceed to creating our user
    // handle verificationCode to be sent to user email
    const verificationCode = crypto.randomInt(100000, 999999).toString(); //six characters randomly generated because it is specificied
    ...
    // we are going to use a process from node process.nextTick this allows us not tpo block synchronous operations the api response wont wait for the email to be sent, even if email fails it wont affect the creation of the user
    process.nextTick(() => {
  ...
  login: async (userData, next) => {
    ...
    const user = await User.findOne({ email: userData.email }).select(
      "+password"
    ); //select includes the field name we want to have access to, in this case the password
    if (!user) {
  ...
  authenticateUser: async (userId, next) => {
    // get userId from jwt decoded token
  ...
  //     path: "/", // cookie is valid on all paths across your domain
  //   });
  ...
  verifyUserAccount: async (data, next) => {
    ...
    console.log("yy", verificationToken);
    ...
    //check if verificationToken saved in the databased is the same as the received from the form
  ...
  resendVerificationToken: async (userId, next) => {
    ...
    process.nextTick(() => {
      mailService.sendVerificationCode(user).catch(async (error) => {
        console.error("Failed to send verification toekn", error);
...
process.nextTick(()=>{
  mailService.sendPasswordResetEmail(user).catch(async (error)=>{
    ...
    await user.save();
    console.error("Failed to send password toekn", error);
  });
});
return user;
  },
